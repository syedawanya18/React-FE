...
const Board = () => {
  ...
  useEffect(()=>{
    ...
    if (sessionStorage.getItem("role")=="sales"){
      console.log(localStorage.getItem("id"))
      fetch(`${api}get/sale/project/${localStorage.getItem("id")}`)
      .then((res) => res.json())
      .then((data) => {
        const cardsWithStringIds = data.map(card => {
          return { ...card, id: card.id.toString() };
        });
        setCards(cardsWithStringIds);
      }
...
const Card = ({
  ...
  return (
    <>
      ...
      >
        <h1 className="text-neutral-100 font-bold text-center">{title}</h1>
        <div className="mt-2">
          ...
          {showPopup && (
            <div className="fixed inset-0 bg-gray-500 bg-opacity-50 flex items-center justify-center">
              <div className="bg-gray-500 p-8 rounded shadow-lg">
                ...
                <p className="mt-2 text-slate-100">{description}</p>
                <button
        ...
        <div className="mt-2 flex items-center">
          <label className="text-xs text-neutral-400">Uploaded by:</label>
          <span className="bg-neutral-800 p-1 text-xs text-neutral-100">
            {bid_by.username}
          </span>
        </div>
